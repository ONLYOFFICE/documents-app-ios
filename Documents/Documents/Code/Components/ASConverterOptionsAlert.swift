...
class ASCConverterOptionsAlert: NSObject, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    func show(_ handler: @escaping (_ encoding: Int, _ delimiter: Int) -> Void) {
        ...
        if !isOnlyCodePages {
            let encodingLabel = UILabel(frame: CGRect(x: 15, y: -5,
            ...
            let delimiterLabel = UILabel(frame: CGRect(x: preferredContentSize.width * 0.5, y: -5,

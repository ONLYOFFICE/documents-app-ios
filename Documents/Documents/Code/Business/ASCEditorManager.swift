...
class ASCEditorManager: NSObject, DEEditorDelegate, SEEditorDelegate, PEEditorDelegate, UITextFieldDelegate, UIDocumentInteractionControllerDelegate {
    ...
    private var lockedHandler: ASCEditorManagerLockedHandler? = nil
    private var openedlocallyFile: ASCFile? = nil
    ...
    private var encoding: Int? = nil
    private var delimiter: Int? = nil
    ...
    func openEditorInCollaboration(file: ASCFile, viewMode: Bool = false, handler: ASCEditorManagerOpenHandler? = nil) {
        ...
        let documentInfo: [String: Any] = [
            ...
            "docKey"                : self.documentKeyForTrack ?? "",
            "docURL"                : self.documentURLForTrack ?? "",
    ...
    public func fetchDocumentService(_ handler: @escaping (String?, String?, Error?) -> Void) {
        ASCOnlyOfficeApi.get(ASCOnlyOfficeApi.apiDocumentService, parameters: ["version": "true"], completion: { (results, error, response) in
    ...
    private func convertToEdit(file: ASCFile, processing: ASCFileManagerConverterHandler? = nil) {
        ...
        if (["csv", "txt"].contains(fileExtension)) && encoding == nil && delimiter == nil {
            ...
            self.alert.isOnlyCodePages = fileExtension == "txt"
            ...
            if let file = self.openedlocallyFile {
                if let key = documentKeyForTrack {
                    ASCOnlyOfficeApi.get(String(format: ASCOnlyOfficeApi.apiFileTrackEdit, file.id),
                               parameters: ["docKeyForTrack": key, "isFinish": "true"]) { (results, error, response) in
                                if let error = error {
                                    log.error(error)
                                }
                    }
    ...
    @objc func updateLocallyEditFile(timer: Timer) {
        ...
        if let file = self.openedlocallyFile,
            ...
            if trackingReadyForLocking == trackingFileStatus {
                ...
                ASCOnlyOfficeApi.get(String(format: ASCOnlyOfficeApi.apiFileTrackEdit, file.id),
                           parameters: ["docKeyForTrack": key]) { (results, error, response) in
                ...
                ASCOnlyOfficeApi.post(String(format: ASCOnlyOfficeApi.apiFileStartEdit, file.id),
                            parameters: ["editingAlone": "true"]) { (results, error, response) in
    ...
    func editOnlineFileLocally(_ file: ASCFile, viewMode: Bool = false) {
        ...
        self.openHandler?(.begin, 0.15, nil, &cancel)
    ...
    func editCloud(_ file: ASCFile, viewMode: Bool = false, handler: ASCEditorManagerOpenHandler? = nil, closeHandler: ASCEditorManagerCloseHandler? = nil) {
        ...
        if let url = UserDefaults.standard.value(forKey: ASCConstants.SettingsKeys.collaborationService) as? String {
            ...
            self.fetchDocumentService({ (url, version, error) in
                if cancel {
                    handler?(.end, 1, nil, &cancel)
                    return
                }
    ...
    func browsePdfCloud(for provider: ASCBaseFileProvider, _ pdf: ASCFile, handler: ASCEditorManagerOpenHandler? = nil) {
        ...
        if let viewUrl = pdf.viewUrl {
            ...
            provider.download(viewUrl, to: URL(fileURLWithPath:destination.rawValue)) { progress, result, error, response in
                if cancel {
                    provider.cancel()
                    handler?(.end, 1, nil, &cancel)
                    return
    ...
    func documentLoading(_ controller: DEEditorViewController!, progress value: CGFloat) {
        ...
        if let file = openedFile, !file.device {
            ASCOnlyOfficeApi.post(String(format: ASCOnlyOfficeApi.apiFileStartEdit, file.id)) { (results, error, response) in
                if let error = error {
                    log.error(error)
                }
            }
        ...
        if let file = openedFile {
            ...
            if file.device {
                if nil != document {
                    ...
                    if self.openedlocallyFile == nil && openedCopy {
                        
                        let copyFile = Path.userDocuments + Path.init(file.id).fileName
                        guard let dstPath = ASCLocalFileHelper.shared.resolve(filePath: copyFile) else {
                            self.closeHandler?(.error, 1, nil, nil, &cancel)
                            return
                        }
                        ...
                        if !ASCConstants.FileExtensions.editorImportDocuments.contains(fileExtension) {
                            /// Store original
                            let _ = ASCLocalFileHelper.shared.move(from: Path(file.id), to: Path.userTemporary + file.title)
                        } else {
                            ...
                            self.resolvedFilePath = filePath
                        }
                    }
                    ...
                    convertToSave(file: file, password: document.password, processing: { status, progress, error, outputPath in
                        if status == .begin {
                            ...
                            // Restore original
                            let _ = ASCLocalFileHelper.shared.move(from: Path.userTemporary + file.title, to: Path(file.id))
                    ...
                    // No changes
                    ASCLocalFileHelper.shared.removeDirectory(Path.userAutosavedInformation + file.title)
                    self.closeHandler?(.end, 1, nil, nil, &cancel)
                ...
                if let closeHandler = self.closeHandler {
                    ...
                    ASCOnlyOfficeApi.get(String(format: ASCOnlyOfficeApi.apiFileId, file.id)) { (results, error, response) in
                        if let results = results as? [String: Any] {
...
//            "asc.pe.external.helpurl": "http://helpcenter.onlyoffice.com/%@%@mobile-applications/documents/presentation-editor/index.aspx"
    ...
    func showConverterOptionsAlert(handler: @escaping (_ encoding: Int, _ delimiter: Int) -> ()) {
